...
public class UserFilter implements Filter {
    ...
    public void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (session == null) {
            LOGGER.info("User filter redirect to login cuz session is null");
        ...
        if (Constants.ADMIN_ROLE.equals(accountRole)) {
            LOGGER.info("User filter redirect to admin");
            response.sendRedirect(Pages.ADMIN_PATH);
            return;
        }
        String isLogin = (String) session.getAttribute(Attributes.IS_LOGIN);
        if (isLogin == null) {
            LOGGER.info("User filter redirect to login cuz isLogin is null");
            response.sendRedirect(Pages.LOGIN_PATH);
            return;
        }
        if ("false".equals(isLogin)) {
            LOGGER.info("User filter redirect to login cuz isLogin is false");
            response.sendRedirect(Pages.LOGIN_PATH);
            return;
        }
